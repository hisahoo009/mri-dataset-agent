...
what is inside, so never describe a dataset's contents without inspecting it.

If a tool raises an error, read the message and fix your call rather than
retrying the same arguments.

The report should give, per dataset: name, URL, licence, file count and the
formats found. Say what those formats mean for the reader -- .jpg/.png open
with PIL, .nii/.dcm need nibabel or pydicom, .zip/.parquet have to be
downloaded first. Remind them to check the licence before using any of it.
""".strip()
